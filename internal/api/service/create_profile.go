...
func (s Service) CreateOwnProfile(ctx context.Context, req *svc.CreateProfileRequest) (*svc.Profile, error) {
	...
	if err != nil {
		Log(ctx, meta.RequestID).WithError(err).Error("failed to create profile and cabinet")

		return nil, responses.AppError(ctx, meta.RequestID, err)
	}

	Log(ctx, meta.RequestID).Infof("created profile and cabinet for user %s", meta.InitiatorID)
