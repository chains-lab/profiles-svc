...
func TestProfiles(t *testing.T) {
	...
	first, err := s.app.CreateProfile(ctx, firstID, "first")
	if err != nil {
		t.Fatalf("CreateProfile first: %v", err)
	}

	second, err := s.app.CreateProfile(ctx, secondID, "second")
	if err != nil {
	...
	first, err = s.app.GetProfileByUserID(ctx, firstID)
	if err != nil {
	...
	second, err = s.app.GetProfileByUserID(ctx, secondID)
	if err != nil {
	...
	first, err = s.app.UpdateProfile(ctx, firstID, app.UpdateProfileInput{
		Avatar:      &avatar,
		Pseudonym:   &newFirst,
		Description: &description,
	})
	...
	second, err = s.app.ResetUserProfile(ctx, secondID)
	if err != nil {
	...
	first, err = s.app.ResetUsername(ctx, firstID)
	if err != nil {
	...
	first, err = s.app.UpdateProfileOfficial(ctx, firstID, false)
	if err != nil {
	...
	first, err = s.app.UpdateProfileOfficial(ctx, firstID, true)
	if err != nil {
